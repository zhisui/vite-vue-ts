...
const preventDefaultExceptionAssert = (
    element: any,
    exceptions: IWheelEmulatorOptions['preventDefaultException']
) =>
...
export default (
    ...
    const wheelEndDetector = (event: ICompatibleWheelEvent) => {
        ...
        wheelEndTimer = window.setTimeout(() => {
            ...
            if (
                wheelCount < 2 &&
                (Math.abs(deltaCache.x) < deltaXStep || Math.abs(deltaCache.y) < deltaYStep)
            ) {
                //防止鼠标滚动距离太小
